...
import { Film, Tv, Calendar, Search } from "lucide-react";
...
const CastMemberFilmography = ({ movieCredits, tvCredits, isMobile }: CastMemberFilmographyProps) => {
  ...
  const filteredCredits = React.useMemo(() => {
    ...
    if (searchTerm) {
      allCredits = allCredits.filter(credit =>
        ...
        credit.character_name?.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
  ...
  return (
    <div className="space-y-4 h-full flex flex-col">
      <div className="bg-card/50 backdrop-blur-sm rounded-xl p-[0.1px] border border-border flex-shrink-0">
        <div className="flex flex-col gap-3 p-4">
          <h3 className={`${isMobile ? 'text-xl' : 'text-lg'} font-bold`}>
        ...
        <ScrollArea className="h-full">
          <div className="pr-3">
            {filteredCredits.length === 0 ? (
              ...
              <div className={`grid ${isMobile ? 'grid-cols-3' : 'grid-cols-6 lg:grid-cols-8'} gap-3`}>

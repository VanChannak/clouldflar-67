    ...
    try {
      ...
      if (!user) {
        ...
        const { data, error } = await supabase
          .from('content')
          .select('id, title, overview, content_type, poster_path, backdrop_path, popularity, genre, tmdb_id')
      ...
      for (const item of historyData) {
        ...
        try {
          const endpoint = contentType === 'movie' 
            ...
            : `https://api.themoviedb.org/3/tv/${tmdbId}/credits?api_key=${TMDB_API_KEY}`;
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative p-2 md:p-6 pl-3 md:pl-4 lg:pl-8 pr-3 md:pr-0">
          ...
          <div className="flex flex-row gap-4 md:gap-6 items-start">
            ...
            <div 
              className="flex-shrink-0 w-[140px] md:w-[180px] lg:w-[270px] cursor-pointer group"
              onClick={() => currentContent && handleContentClick(currentContent)}
            >
              <div className="sticky top-6 rounded-lg overflow-hidden bg-muted border-2 border-border group-hover:border-primary transition-all aspect-[2/3] md:aspect-auto md:h-[270px] lg:h-[388px]" style={{ maxHeight: '393px' }}>
                <img
                  src={currentContent?.poster_path || currentContent?.backdrop_path || '/placeholder.svg'}
                ...
                <div className="absolute bottom-0 left-0 right-0 p-3 md:p-4">
                  ...
                  {currentContent?.genre && (
                    <p className="text-gray-300 text-xs md:text-sm mt-1">{currentContent.genre}</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 min-w-0 flex flex-col md:h-[270px] lg:h-[388px]" style={{ minHeight: '255px' }}>
              {/* Recent Watch Cards - Responsive */}
              <div className="flex-1 min-w-0 md:h-[203px] lg:h-[298px]" style={{ minHeight: '170px' }}>
                <div className="relative h-full">
                  {/* Cards Grid - Responsive Columns with Horizontal Scroll */}
                  <div
                    id="continuous-watch-cards"
                    className="grid grid-flow-col auto-cols-[minmax(90%,90%)] md:auto-cols-[minmax(48%,48%)] gap-3 md:gap-6 overflow-x-auto scrollbar-hide h-full"
                  ...
                  >
                    {watchHistory.map((item) => (
                      <div
                        ...
                        className="cursor-pointer group/card border-2 border-border/50 rounded-xl overflow-hidden transition-all duration-300 hover:shadow-2xl hover:border-primary bg-background/60 dark:bg-black/60 backdrop-blur-sm h-full"
                      >
                        <div className="flex gap-3 md:gap-6 p-3 md:p-6 h-full">
                          ...
                          <div className="flex-shrink-0 w-20 md:w-28 lg:w-32">
                            <div className="relative aspect-[2/3] rounded-lg overflow-hidden bg-muted shadow-lg">
                              <img
                                src={item.poster_path || item.backdrop_path || '/placeholder.svg'}
                                alt={item.title}
                          ...
                          <div className="flex-1 min-w-0 flex flex-col justify-center space-y-2 md:space-y-3">
                            ...
                            {item.overview && (
                              <p className="hidden md:block text-xs md:text-sm text-muted-foreground line-clamp-2 md:line-clamp-3 mt-1 md:mt-2 leading-relaxed">
                                {item.overview}
                              </p>
                            )}
                          </div>
              ...
              {castMembers.length > 0 && (
                <div className="mt-2 md:mt-6 md:h-[68px] lg:h-[89px]" style={{ minHeight: '68px' }}>
                  <div className="flex gap-3 md:gap-5 overflow-x-auto scrollbar-hide h-full">
                    {castMembers.map((cast) => (
                      <div
                        key={cast.id}
                        onClick={() => setSelectedCastId(cast.tmdb_id)}
                        className="flex-shrink-0 cursor-pointer group/cast-item flex items-center gap-2 md:gap-3"
                      >
                        <div className="relative w-16 h-16 md:w-20 md:h-20 lg:w-24 lg:h-24 rounded-full overflow-hidden bg-muted border-2 md:border-4 border-border group-hover/cast-item:border-primary transition-all shadow-lg flex-shrink-0">
                          <img
                            ...
                            alt={cast.name}
                            className="w-full h-full object-cover transition-transform duration-300 group-hover/cast-item:scale-110"
                            onError={(e) => {

    ...
    if (action === 'generate') {
      ...
      try {
        ...
        if (checkData.responseCode === 0 && checkData.data?.status === 'COMPLETED') {
          console.log('Payment confirmed by Bakong, updating wallet balance');
          ...
          // Call the add_wallet_funds RPC function
          ...
          console.log('Wallet updated successfully, new balance:', walletData);
          ...
          console.log('Payment not confirmed. Response:', checkData);
          return new Response(
            JSON.stringify({
              success: false,
              status: 'pending',
              message: 'Payment not confirmed yet',
              bakongResponse: checkData.responseMessage || 'Waiting for payment',
            }),
            { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          );
        }
        ...
        // Return pending status instead of error to allow continued polling
        return new Response(
          JSON.stringify({
            success: false,
            status: 'pending',
